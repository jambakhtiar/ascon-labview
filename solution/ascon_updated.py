...
#def labview(variant, invalue):
def labview_encrypt(invalue):
    global variant
    global key
    global nonce
    global associateddata
    global plaintext
    #global ciphertext
    variant =  "Ascon-128"
    assert variant in ["Ascon-128", "Ascon-128a", "Ascon-80pq"]
    ...
    integer_number = int(invalue)
    plaintext      = integer_number.to_bytes((integer_number.bit_length() + 7) // 8)
    print("messaage into encrypt: ", invalue)
    ...
    characters = int.from_bytes(ciphertext, byteorder='big')
    print("message outof encrypt: ", characters)
    float_number = float(characters)
    return float_number
    return ciphertext
'''
...
def labview_decrypt(pciphertext):
    
    #ciphertext        = ascon_encrypt(key, nonce, associateddata, plaintext,  variant)
    receivedplaintext = ascon_decrypt(key, nonce, associateddata, pciphertext, variant)
    ...
    demo_print([("key", key), 
                ...
                ("plaintext", plaintext), 
                ("ass.data", associateddata), 
                ("ciphertext", pciphertext[:-16]), 
                ("tag", pciphertext[-16:]), 
                ("received", receivedplaintext),
               ])

    characters = int.from_bytes(receivedplaintext, byteorder='big')
    print("message out of decrypt: ", characters)
    float_number = float(characters)
    return float_number
    #return ciphertext
    ...
    #demo_aead("Ascon-128")
    #labview("Ascon-128", 10)
#labview("Ascon-128", 2233)

#labview_encrypt(100)
cp = labview_encrypt(100)
labview_decrypt(cp)
